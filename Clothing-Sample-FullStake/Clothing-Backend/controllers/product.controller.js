...
module.exports = {
  createProduct: async (req, res) => {
    try {
      const { productName, price, category_id, description } = req.body;
  ...
  getProducts: async (req, res) => {
    try {
      const { productName, category_id, from_price, to_price, page = 1 } = req.query;
      const bodyQuery = {};
  ...
  updateProduct: async (req, res) => {
    try {
      ...
      const update = await productModel.findByIdAndUpdate(
        id,
        { productName, price, category_id, description },
        { new: true },
      );
      if (update) {
